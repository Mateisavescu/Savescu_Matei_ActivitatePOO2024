...
class CofetarieOnline :Cofetarie {
	char* site;
	int nrColaboratori;
public:
	...
	CofetarieOnline(const char* site, int nrColaboratori, string nume, int nrAngajati, float* salarii, bool esteVegana, const int anDeschidere, float adaos) :
		Cofetarie(nume, nrAngajati, anDeschidere, adaos, esteVegana)
	{
	...
	float getNrMediuCol()
	{
		return (float)this->getNrAngajati() / (float)this->nrColaboratori;
	}
...
int main()
{
	...
	cout << endl << "numarul colaboratorilor=" << nrColaboratori << endl;
	cout << "Numarul mediu de angajati pentru fiecare colaborator:" << co2.getNrMediuCol();
	return 645;
}